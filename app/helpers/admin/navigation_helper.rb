module Admin::NavigationHelper
  ...
  def tab(*args)
    ...
    #Determinamos la ruta dependiendo de rol....
    if current_user.has_role? 'vendor' then prefijo = "vendor" end
    if current_user.has_role? 'cliente' then prefijo = "cliente" end
    if current_user.has_role? 'admin' then prefijo = "admin" end

    options[:route] ||=  "#{prefijo}_#{args.first}"
    ...
    selected = if options[:match_path]
      request.fullpath.starts_with?("/#{prefijo}{options[:match_path]}")
    else
