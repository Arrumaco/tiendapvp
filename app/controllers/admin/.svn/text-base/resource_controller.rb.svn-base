...
class Admin::ResourceController < Admin::BaseController
  ...
  def new_object_url(options = {})
    ...
    if parent_data.present?
      new_polymorphic_url([session[:rol_usuario], parent, model_class], options) 
    else
      new_polymorphic_url([session[:rol_usuario], model_class], options) 
    end
  end
  ...
  def edit_object_url(object, options = {})
    rol = session[:rol_usuario]
    if parent_data.present?
      send "edit_#{session[:rol_usuario]}_#{parent_data[:model_name]}_#{object_name}_url", parent, object, options 
    else
      send "edit_#{session[:rol_usuario]}_#{object_name}_url", object, options 
    end
  end
  ...
  def object_url(object = nil, options = {})
    ...
    if parent_data.present?
      send "#{session[:rol_usuario]}_#{parent_data[:model_name]}_#{object_name}_url", parent, target, optionsd
    else
      send "#{session[:rol_usuario]}_#{object_name}_url", target, options
    end
  end

  def collection_url(options = {})
    if parent_data.present?
      polymorphic_url([session[:rol_usuario], parent, model_class], options)
    else
      polymorphic_url([session[:rol_usuario], model_class], options)
    end
  end
